...
public class DateUtil {
	...
	public static java.sql.Timestamp str2Timestamp(String str, String sFormat) {
		Date date = string2Date(str, sFormat);
	 ...
	 * 
	 * @param type
	 *            间隔时间单位类型，取值范围为：yyyy、m、d、h、n、s，对应为年、月、日、时、分、秒
	 * @param fromDate
	 *            起始时间
	 * @param toDate
	 *            终止时间
	...
	public static long getQuot( long date1, long date2 ) {
		  long quot = 0;
		  quot = date1 - date2;
	 ...
	 *            前/后偏移数量
	 * @param strOffsetUnit
	 *            前/后推周期单位 day,week,month,year,hour
	 * @return Calendar
	 * 
	 */
	...
	public static Date getOffsetDate(String strYYYYMMDD, int nOffsetNum,
		...
		System.out.println("ssss==="+caldTmp.getInstance().getTime());
	...
	/**
	 * 取得指定分隔符分割的年月日的日期对象,去掉分割符.
	 * 
	 * @param argsDate
	 *            格式为"yyyy-MM-dd"等格式
	 * @param split
	 *            时间格式的间隔符，例如“-”，“/”，要和时间一致起来。
	 * @return 一个String类型的对象
	 */
	public static String getDateString(String argsDate, String split) {
		String[] temp = argsDate.split(split);
	...
	/**
	 * 获取当前时间年月日小时分秒毫秒
	 * @return 年 + 月 + 日 + 小时 + 分 + 秒 + 毫秒
	 */
	public static String getCurrentTime() {
		  Calendar CD = Calendar.getInstance();
		  StringBuilder builder = new StringBuilder();
		  builder.append( CD.get(Calendar.YEAR) )
		         .append( CD.get(Calendar.MONTH)+1 )
		         .append( CD.get(Calendar.DATE) )
		         .append( CD.get(Calendar.HOUR) )
		         .append( CD.get(Calendar.MINUTE) )
		         .append( CD.get(Calendar.SECOND) )
		         .append( CD.get(Calendar.MILLISECOND) );
	...
	public static String getCalendarDate(int day) {
		//获取当前日期前15天
